...
def get_tvid_duration_title(html_url):
    ...
    script_html = list(
        filter(
            lambda a: "window.QiyiPlayerProphetData" in a.text, soup.findAll("script")
        )
    )[0]
    vid_script = json.loads(script_html.text.split("window.QiyiPlayerProphetData=")[1])
    tvid = str(vid_script["tvid"])
    video_duration = vid_script["a"]["data"]["showResponse"]["videoInfo"][
        "videoDuration"
    ]
...
if __name__ == "__main__":
    ...
    danmu = get_iqiyi_danmus(
        tvid_duration_title["tvid"], tvid_duration_title["duration"]
    )
